...
// ReSharper restore PossibleNullReferenceException
        ...
        public IEnumerable<Fisk> GetAllFishFromDay(DateTime date)
        {
            ...
            //return _repo.Fisks.Where(s => s.DatoNew.Date.Equals(date.Date));
        }
        ...
        public IEnumerable<Fisk> GetAllFishFromDay(DateTime date, int havnId)
        {
            ...
            //return _repo.Fisks.Where(s => s.DatoNew.Date.Equals(date.Date) && s.Havne.Id == havnId);
        }
        ...
        public IEnumerable<Fisk> GetAllFishNewest()
        {
            ...
            var dateQuery = _repo.Fisks.Select(o => new FishDateTimeClass
            {
                //DATENEW
            ...
            var returnFish = tst.Select(s => s.Fisk);
        ...
        public IEnumerable<Fisk> GetAllFishNewest(int havneId)
        {
            ...
            */
            ...
            if (firstOrDefault == null) return null;
            var tst = fisk.DistinctBy(s => new {s.Fisk.Arters.Navn, s.Fisk.Sort});
            //var returnFish = tst.ToList().Select(s => s.Fisk);
...
// ReSharper restore LoopCanBeConvertedToQuery
        ...
        {
            _repo.Nyhedes.Remove(_repo.Nyhedes.FirstOrDefault(s => s.Id == id));
            Save();
        }

        public Nyhede GetBlog(int id)
        {
            return _repo.Nyhedes.FirstOrDefault(s => s.Id == id);
        }

        public void UpdateBlog(Nyhede blog)
        {
            var bg = _repo.Nyhedes.FirstOrDefault(s => s.Id == blog.Id);
            bg.Body = blog.Body;
            bg.Id = blog.Id;
            bg.ImageLink = blog.ImageLink;
            bg.Title = blog.Title;
            bg.Date = blog.Date;
            _repo.SaveChanges();
        }
